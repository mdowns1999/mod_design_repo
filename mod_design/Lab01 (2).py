# 1. Name:
#      Mike Downs
# 2. Assignment Name:
#      Lab 01: Tic-Tac-Toe
# 3. Assignment Description:
#      Play the game of Tic-Tac-Toe
# 4. What was the hardest part? Be as specific as possible.
#      The hardest part of this assignment was figuing out how to write and read to the file.  Thankefully, I recieved some help from the TA on how to accomplish this task.  
# I learned a lot about that sort of thing and I think that it is pretty cool.  Once I figured out how to write and read a file, the next hardest part was resetting and returning to the board.
#When I figured that out, everything else seemed to fall into place.
# 5. How long did it take for you to complete the assignment?
#      3 Hours

import json

# The characters used in the Tic-Tac-Too board.
# These are constants and therefore should never have to change.
X = 'X'
O = 'O'
BLANK = ' '

# A blank Tic-Tac-Toe board. We should not need to change this board;
# it is only used to reset the board to blank. This should be the format
# of the code in the JSON file.
blank_board = {  
            "board": [
                BLANK, BLANK, BLANK,
                BLANK, BLANK, BLANK,
                BLANK, BLANK, BLANK ]
        }


def read_board(filename):
    '''Read the previously existing board from the file if it exists.'''
    try:
        with open(filename, 'r') as file:
            # Put file reading code here.
            return json.load(file)['board']
    except:
        return blank_board['board']


def save_board(filename, board):
    '''Save the current game to a file.'''
    #DUMP SAVES TO FILE. DUMPS you need to use write function
    # Put file writing code here.
    with open(filename, 'w') as file:
        # Put file reading code here.
        json.dump({'board': board},file)


def display_board(board):
    '''Display a Tic-Tac-Toe board on the screen in a user-friendly way.'''
    # Put display code here.
    
    print(f'{board[0]} | {board[1]} | {board[2]}')
    print('- + - + -')
    print(f'{board[3]} | {board[4]} | {board[5]}')
    print('- + - + -')
    print(f'{board[6]} | {board[7]} | {board[8]}')

    return board
    

def is_x_turn(board):
    '''Determine whose turn it is.'''
    # Put code here determining if it is X's turn.
    if board.count('O') < board.count('X'):
        return False
    else:
        return True

def play_game(board):
    '''Play the game of Tic-Tac-Toe.'''
    # This block of code will check if the current board is blank or not. If it is not blabk, we will prompt the usser if they want to return to the saved game.
    return_input = ''
    if board != blank_board['board']:
        return_input = input('Would you like to return to the previous game? Y or N?')
        if return_input.upper() == 'Y':
            print('Enjoy the rest of your Game!')
        else:
            return False


    user_input = ''
    
    while user_input != 'q':

        #This loop checks the user's input.
        input_valid = False
        while not input_valid:
            user_input = input("Enter 'q' to suspend your game. Otherwise, enter a number from 1 to 9 where the following numbers correspond to the locations on the grid: ")

            if user_input.isnumeric():
                user_input = int(user_input)
                if 1 <= user_input and user_input <= 9:
                    input_valid = True
            elif user_input == 'q':
                save_board('game.json', board)
                input_valid = True
                return False
            else:
                print('Please Try Again')

        # THis calls the is_x_turn function to see if X or O goes
        if is_x_turn(board) == True:
            board[user_input-1] = X
        else:
            board[user_input-1] = O
        
        display_board(board)

        #The function checks if the board has any winning rows or columns.
        if game_done(board):
            user_input = 'q'
            save_board('game.json', blank_board['board'])
        elif user_input == 'q':
            save_board('game.json', board)

    return False

def game_done(board, message=False):
    '''Determine if the game is finished.
       Note that this function is provided as-is.
       You do not need to edit it in any way.
       If message == True, then we display a message to the user.
       Otherwise, no message is displayed. '''

    # Game is finished if someone has completed a row.
    for row in range(3):
        if board[row * 3] != BLANK and board[row * 3] == board[row * 3 + 1] == board[row * 3 + 2]:
            message = True
            if message:
                print("The game was won by", board[row * 3])
            return True

    # Game is finished if someone has completed a column.
    for col in range(3):
        if board[col] != BLANK and board[col] == board[3 + col] == board[6 + col]:
            message = True
            if message:
                print("The game was won by", board[col])
            return True

    # Game is finished if someone has a diagonal.
    if board[4] != BLANK and (board[0] == board[4] == board[8] or
                              board[2] == board[4] == board[6]):
        message = True
        if message:
            print("The game was won by", board[4])
        return True

    # Game is finished if all the squares are filled.
    tie = True
    for square in board:
        if square == BLANK:
            tie = False
    if tie:
        if message:
            print("The game is a tie!")
        return True


    return False

# These user-instructions are provided and do not need to be changed.
print("Enter 'q' to suspend your game. Otherwise, enter a number from 1 to 9")
print("where the following numbers correspond to the locations on the grid:")
print(" 1 | 2 | 3 ")
print("---+---+---")
print(" 4 | 5 | 6 ")
print("---+---+---")
print(" 7 | 8 | 9 \n")
print("The current board is:")


# The file read code, game loop code, and file close code goes here.
board = read_board('game.json')
display_board(board)
play_game(board)